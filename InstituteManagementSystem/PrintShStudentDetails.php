...
<div class="container"><center><h3>Short Studnet Details</h3></center></div>
<?php
session_start();
$Name=$_SESSION['fname'];
...
$result=mysqli_query($con,"SELECT `ID` FROM `shstudentfrom` WHERE `First Name` = '$Name'");
...
$con=mysqli_connect("localhost","root","");
mysqli_select_db($con,"iceproject");
$result=mysqli_query($con,"SELECT *  FROM `shstudentfrom` where `ID`= '$id'");
...
while($row = mysqli_fetch_array($result))
{
	...
	echo "<img src='Stu_Tea_Image/".$row['Short Student Adhar']." ' width='100px' height='100px' >";
	echo "</td>";
	
	echo "<th>Student Image</th>";
	echo "<td>";
	echo "<img src='Stu_Tea_Image/".$row['Short Student Image']." 'width='100px' height='100px'>"; 
	echo "</td></tr>";
	echo "</table>";
	echo "</div>";
